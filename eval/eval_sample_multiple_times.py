...
from utils.utils_function import (
    ...
    post_process_batch_data_eval_sample_multiple_times,
)
...
def evaluation_batch(model_path, model, dataset, output_dir, raw_data_path, accuracy_path,
    ...
    for batch_start in tqdm.tqdm(range(0, len(dataset), batch_size)):
        ...
        try:
            ...
            tqdm.tqdm.update(1)
            continue 
        batch_rationales, correct_num, total_num, accuracy = post_process_batch_data_eval_sample_multiple_times(batch_prompts, batch_items, batch_responses, mode, total_num, correct_num, model, max_tokens, temperature, top_p, top_k, stop, is_chat_model, prompt_mode=prompt_mode)
